    ...
    with open(filename,'w') as f:
        ...
        if dimen == 1:
            ...
            if issubclass(type(ndarray[0]),int):
                ...
                for row in ndarray:
                    print >> f, '(%.15e,%-.15e)'%(row.real,row.imag)                
            ...
            if issubclass(type(ndarray[0,0]),int):
                ...
                print >> f, 'ARMA_MAT_TXT_FN008\n%d %d'%(shape[0],shape[1])
                for row in ndarray:
                    print >> f, ' '.join('%.15e'%x for x in row)
            elif issubclass(type(ndarray[0,0]),complex):
                print >> f, 'ARMA_MAT_TXT_FC016\n%d %d'%(shape[0],shape[1])
                for row in ndarray:
                    print >> f, ' '.join('(%.15e,%-.15e)'%(x.real,x.imag) for x in row)                
...
def arma2json (ndarray):
    ...
    if dimen == 1:
        ...
        if issubclass(type(ndarray[0]),int):
            data = {"nrow":shape[0],"real":ndarray.tolist()}
            return data
        elif issubclass(type(ndarray[0]),float):
            data = {"nrow":shape[0],"real":ndarray.tolist()}
            return data
        elif issubclass(type(ndarray[0]),complex):
            data = {"nrow":shape[0],"real":ndarray.real.tolist(),"imag":ndarray.imag.tolist()}
            return data
    
    elif dimen == 2:

        if issubclass(type(ndarray[0,0]),int):
            data = {"nrow":shape[0],"ncol":shape[1],"real":ndarray.ravel().tolist()}
            return data
        elif issubclass(type(ndarray[0,0]),float):
            data = {"nrow":shape[0],"ncol":shape[1],"real":ndarray.ravel().tolist()}
            return data
        elif issubclass(type(ndarray[0,0]),complex):
            data = {"nrow":shape[0],"ncol":shape[1],"real":ndarray.real.ravel().tolist(),"imag":ndarray.imag.ravel().tolist()}
            return data
    
    elif dimen == 3:
    
        if issubclass(type(ndarray[0,0,0]),int):
            data = {"nrow":shape[1],"ncol":shape[2],"nslc":shape[0],"real":ndarray.ravel().tolist()}
            return data
        elif issubclass(type(ndarray[0,0,0]),float):
            data = {"nrow":shape[1],"ncol":shape[2],"nslc":shape[0],"real":ndarray.ravel().tolist()}
            return data
        elif issubclass(type(ndarray[0,0,0]),complex):
            data = {"nrow":shape[1],"ncol":shape[2],"nslc":shape[0],"real":ndarray.real.ravel().tolist(),"imag":ndarray.imag.ravel().tolist()}
            return data
